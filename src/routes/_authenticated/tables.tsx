...
function Page() {
  ...
  async function openTableForOrder(t: any) {
    if (!user) return;
    // Find existing open order on this table, or create one
  ...
  async function doTransfer() {
    ...
    // Move open order from source to target, only if target available
    ...
    await supabase.from("dining_tables").update({ status: "occupied" }).eq("id", transferTo);
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {tables.filter((t) => t.area_id === a.id).map((t) => (
                <Card key={t.id} className={cn(
                  ...
                  t.status === "available" && "border-success/30 bg-success/5",
                  t.status === "occupied" && "border-primary bg-primary/10",
                  t.status === "reserved" && "border-warning/40 bg-warning/10",
                  ...
                  <div className={cn("text-xs mt-2 font-medium capitalize",
                    t.status === "occupied" && "text-primary",
                    t.status === "available" && "text-success",
                    t.status === "reserved" && "text-warning-foreground",
                  )}>{t.status}</div>
                  {t.status === "occupied" && (
                    <Button variant="ghost" size="sm" className="mt-2 h-7 px-2" onClick={(e) => { e.stopPropagation(); setTransferFrom(t); }}>
